...
function change(game){
    ...
    window.side = document.createElement("div");
    let el = `
        <div id="mySidenav" class="sidenav">收起<<<<<<<<<<<<<<<<<<<<<<<<<</div>
        <style>
            .side-btn{
                background-color: rbg(40，156，244);
                background-radius: 0;
                background-insets: 0;
            }

            .side-btn:hover{
                background-color: rgb(82, 178, 246);
            }
        </style>
        <button id= "add" type="button" class="side-btn">自己速度增加两倍</button>
        <button id= "otherAdd" type="button" class="side-btn">敌人速度增加两倍</button>
        <button id= "sub" type="button" class="side-btn">自己速度减少两倍</button>
        <button id= "otherSub" type="button" class="side-btn">敌人速度减少两倍</button>
    `
    side.innerHTML = el;
    ...
    var otherAddBut = document.getElementById('otherAdd');
    otherAddBut.onclick = function(){
        console.log("增加两倍");
        game.enemyNode.children.forEach((item) => {
                        item.speed *= 2;
                    })
    };
    ...
    var otherSubBut = document.getElementById('otherSub');
    otherSubBut.onclick = function(){
        console.log("减少两倍");
        game.enemyNode.children.forEach((item) => {
                        item.speed /= 2;
                    })
    };
    var sideBut = document.getElementById('mySidenav');
    sideBut.onclick =  function(){
        const panel = window.side;
        console.log("收起");
        if (window.flag === true) {
